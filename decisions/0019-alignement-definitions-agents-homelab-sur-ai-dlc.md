...
- **IMP-004** : Pointeurs & numérotation — le présent ADR est numéroté **0019**. **Collision de numérotation résolue** : rédigé en parallèle sur une branche séparée, il avait d'abord pris `0018` ; or le Stage 7 (ALI-207) a fusionné sur `main` un ADR `0018` (`0018-adaptation-modele-conductor-stages-protocols-homelab.md`) avant l'intégration de ce stage. Au rebasage sur `main`, le présent ADR a été renuméroté **0018 → 0019** (même stratégie qu'[ADR-0011](0011-alignement-memoire-de-regles-sur-ai-dlc.md), IMP-004). Les pointeurs `README.md` et `AGENTS.md` sont mis à jour vers `0001…0019`. La renumérotation fine de la plage intermédiaire du README relève de la vérification globale (ALI-214).
