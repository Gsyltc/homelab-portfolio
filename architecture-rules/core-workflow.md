...
Ce workflow est le contrat commun d'orchestration **multi-agents (A2A)** des travaux d'architecture du workspace. Il adapte le workflow adaptatif AWS AI-DLC à la gouvernance réelle du workspace, coordonnée par **Sylvain — Architecture Solution & Intégration**.
...
Le workflow n'est pas exécuté par un seul agent. **Sylvain est le coordinateur** : il analyse la demande, découpe en livrables, délègue aux agents spécialisés via des mentions sur les issues, contrôle les livrables, sollicite la sécurité, puis demande la validation humaine granulaire. **Sylvain ne produit pas lui-même les livrables** (sauf vérification).

### Acteurs et responsabilités

| Acteur                                            | UUID                                          | Rôle dans le workflow                                                                                                                                                              |
| ------------------------------------------------- | --------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **Humain (demandeur / valideur)**                 | membre `0da322ff-1018-4729-9511-24e60d53dcbe` | Exprime le besoin, arbitre les choix, valide **chaque** décision architecturale (validation granulaire) et autorise les actions à impact.                                          |
| **Sylvain — Architecture Solution & Intégration** | `713b64a4-98f6-4cec-949a-e1521bd37d51`        | **Coordinateur**. Lance et supervise les travaux, contrôle la cohérence avec les ADR, sollicite Xavier pour la sécurité, demande les validations humaines, orchestre la livraison. |
| **Manuel — Architecte de solution**               | `992ce2c8-aaba-4592-9702-dc47786e64ab`        | Documentation d'architecture (DAS), ADR, diagrammes C4 / Archimate / PlantUML / CALM. Ne traite pas la cybersécurité.                                                              |
| **Xavier — Architecte cybersécurité**             | `694a1a6f-9659-48ea-b45f-43ae6dc01706`        | Analyse des risques (OWASP, STRIDE, NIST, COBIT ; PCI DSS / GDPR / Loi 25 / LPRPDE sur demande explicite). **Sollicité par Sylvain à chaque modification d'architecture.**         |
| **Florian — Architecte AWS**                      | `84e04027-7d53-4013-b09a-5c7cfc978699`        | Services AWS, diagrammes AWS, estimation et optimisation des coûts (tarifs officiels sourcés). Intervient quand AWS est requis.                                                    |
| **Admin — Infrastructure Windows**                | `c1b4db07-a7b8-42d7-998a-0fc54aba630b`        | Administration Windows : migrations Win10→Win11, Intune, VM, golden images, Autopilot, SCCM. Plan de rollback validé avant toute action destructive.                               |
| **Fabien — OpenSpec Expert**                      | `c2dbee8f-9ed4-4867-9b21-6cdd4a8840eb`        | Cycle spec-driven (proposition, implémentation, archivage). **Sollicité uniquement si OpenSpec est activé** pour le projet.                                                        |
| **Nina — Experte d'archivage**                    | `8f54de1e-9725-4c0a-9dc7-9bb32f160acb`        | Import / export de fichiers, archives, mise à disposition et téléchargement des livrables validés.                                                                                 |
| **Alfred — Notifications**                        | `9b5a4076-7b9c-4db6-9d03-06ba49ae0f0f`        | Notification (ntfy) de fin de tâche, sur demande de Sylvain.                                                                                                                       |
...
- Exemple : les instructions détaillées d'un architecte (Manuel, Florian, Xavier, Admin) ne sont pertinentes que pour l'agent lui-même au moment de sa tâche ; le coordinateur se contente de la description pour router.
...
- **Aucune méthodologie (ou explicitement aucune)** → suivre le **parcours d'architecture standard** décrit ci-dessous (documentation d'architecture + ADR + diagrammes produits par Manuel / Florian / Admin, sans cycle méthodologique spécifique).

> Quelle que soit la méthodologie, la **gouvernance A2A reste identique** : coordination par Sylvain, contrôle sécurité systématique par Xavier, ADR obligatoires, validation humaine granulaire, mise à disposition via Nina, notification via Alfred.
...
flowchart TD
    ...
    B -.->|securite Xavier + validation granulaire humaine| B
    C -.->|securite Xavier + validation granulaire humaine| C
```
...
**Livrable de sortie** : conception cible et décisions (ADR) **validées par l'humain** de façon granulaire, après contrôle sécurité par Xavier.
...
2. **Découper le travail en livrables** et désigner l'agent responsable de chacun :
   - Documentation d'architecture / ADR / diagrammes → **Manuel**.
   - Choix AWS, diagrammes AWS, coûts → **Florian** (si AWS requis).
...
2. **Contrôle sécurité obligatoire (Xavier)** : à **chaque modification d'architecture** par un agent, Sylvain lit le résumé des modifications, poste un commentaire mentionnant **Xavier** (`694a1a6f-9659-48ea-b45f-43ae6dc01706`) avec le contexte, **attend son analyse** et intègre ses recommandations avant toute validation. Préciser explicitement toute norme spécifique à appliquer (PCI DSS, GDPR, Loi 25, LPRPDE) — sinon seules OWASP/STRIDE (+ NIST/COBIT si documentation des risques) sont actives.
...
1. **Solliciter Xavier** pour tout livrable modifiant l'architecture (mêmes règles qu'en 1.5).
...
Une fois la tâche réalisée et passée en revue, Sylvain demande à **Alfred** (`9b5a4076-7b9c-4db6-9d03-06ba49ae0f0f`) d'envoyer une notification (ntfy) : message court (« L'issue a été réalisée »), identifiant de l'issue et lien si possible.
...
- **Sécurité systématique** : toute modification d'architecture passe par Xavier avant validation humaine ; normes spécifiques appliquées uniquement si explicitement demandées.
...
- **Séparation des responsabilités** : fichiers/exports via Nina ; notifications via Alfred ; Docker Compose hors périmètre (Stuart).
...
sequenceDiagram
    ...
    participant A as Manuel / Florian / Admin (ou Fabien si OpenSpec)
