...
- **`contexte-client`** : gestion du **référentiel des contextes clients** `${ROOT_DIRECTORY}/clients/<nom-client>.json` (source unique : schéma, nommage, règles de maintenance). Le Gestionnaire CV en est le **producteur** : lors de l'analyse d'un **CV long contenant un contexte client**, il **crée/complète** le fichier du client (contexte de la société **enrichi sans écrasement aveugle** + mandats réalisés **dédoublonnés**) — **ne rien inventer**. Alimente l'expertise de firme de l'Analyste RFP et le bloc « Contexte de l'organisation » du CV long.
