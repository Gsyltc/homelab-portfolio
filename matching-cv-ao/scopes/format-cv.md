...
- **`chargement-cv`** (Initialisation) — repérage / inventaire des CV sources à traiter dans `cv/originaux/` (dépôt temporaire) et des analyses existantes.
- **`extraction-cv`** (Analyse) — lecture des CV sources (`cv/originaux/`), extraction des informations
  ...
  fiches déplacées dans `cv/archives/`) et du **JSON d'analyse versionné**, puis **suppression des fichiers
  sources PDF/DOCX** — le tout daté du **jour**.
...
> (dépôt **temporaire** des sources PDF/DOCX, **supprimées après extraction** — non conservées) ·
> `cv/archives/` (anciennes fiches Markdown) · fiche Markdown du jour et JSON versionnés à la racine.
