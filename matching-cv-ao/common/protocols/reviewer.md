...
- Vérifie : correspondance exigences ↔ profils, absence de critère non tracé, absence de profil orphelin, respect des conventions (format JSON, **communication agent↔agent par fichier JSON joint** — commentaire minimal = mention active + nom de fichier —, présentation agent↔humain **Markdown détaillée aux gates** ; voir `governance-security` « Règle A2A »).
