...
En toute fin de tâche, l'**Analyste RFP** **téléverse un JSON de retour** (type `retour` du message A2A — résultat + artefacts produits) et clôt son commentaire **minimal** par un **lien de mention actif** vers le **Coordinateur Matching** (l'assigneur) + le nom du fichier JSON joint : `[@Coordinateur Matching](mention://agent/<uuid>) — resume-ao.json`. **Aucune reformulation du livrable en prose.**
...
- **Seul ce lien actif enqueue le run de reprise du Coordinateur.** Une adresse en texte clair (« à toi ») n'enqueue **aucun** run (incident EXPE-58).
