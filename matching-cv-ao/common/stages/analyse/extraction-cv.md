...
- produire le JSON `collaborateurs` (voir la compétence [`../../../skills/cv-analyse/SKILL.md`](../../../skills/cv-analyse/SKILL.md) pour la sélection de la dernière version JSON pour le matching et la journalisation d'audit) ;
...
- **Garde-fou — ne pas transmettre les CV** : au coordinateur, ne remonter que le verdict `eligibilite` + la référence `analyse_json` des retenus. Ni sources, ni fiches Markdown, ni JSON complet ne circulent (voir la compétence [`../../../skills/cv-analyse/SKILL.md`](../../../skills/cv-analyse/SKILL.md), `## Garde-fou — non-transmission des CV`).
