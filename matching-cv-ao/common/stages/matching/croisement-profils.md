...
En toute fin de tâche, le **Matcher Profils** **téléverse un JSON de retour** (type `retour` du message A2A — scores + classement, artefact `matching-resultats`) et clôt son commentaire **minimal** par un **lien de mention actif** vers le **Coordinateur Matching** (l'assigneur) + le nom du fichier JSON joint : `[@Coordinateur Matching](mention://agent/<uuid>) — matching-resultats.json`. **Aucune reformulation du livrable en prose.**
