...
Boucle d'apprentissage maison (voir [`core/rules/`](../../../rules/README.md)) : tracer sur l'issue les candidats-règles (motifs d'auto-détection, corrections de scope) ; les remonter au **gate humain** léger d'Ideation ; persistance des apprentissages **confirmés** dans `core/rules/` (couche `scope` incluse) via le cycle capture → confirmation humaine → contrôle de conflit.
