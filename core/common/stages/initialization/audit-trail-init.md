...
Boucle d'apprentissage maison (voir [`core/rules/`](../../../rules/README.md)) : ce stage **ouvre** le journal des candidats-règles sur l'issue et charge les règles persistantes (chargement paresseux). Stage d'Initialization (bootstrap déterministe) → **saute** l'interaction liée au gate humain ; l'écriture de règles reste réservée au cycle capture → confirmation humaine → contrôle de conflit.
