...
## Learn
Boucle d'apprentissage maison (voir [`core/rules/`](../../../rules/README.md)) : ce stage **ouvre** le journal des candidats-règles sur l'issue et charge les règles persistantes (chargement paresseux). Stage d'Initialization (bootstrap déterministe) → **saute** l'interaction liée au gate humain ; l'écriture de règles reste réservée au cycle capture → confirmation humaine → contrôle de conflit. Divergence tracée vs un journal `memory.md` externe (voir [ADR-0009](../../../../decisions/0009-alignement-fiches-de-stage-sur-ai-dlc.md)).
