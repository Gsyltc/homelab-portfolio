...
- **Données HORS périmètre de production détaillée** : l'analyse des données (cycle de vie, gouvernance, classification) et le document `documentation/10-cycle_vie_donnees.md` appartiennent à l'**Architecte de données (Diego)**. **Délègue-lui** les tâches relatives aux données au besoin (mention A2A, UUID résolu, mission cadrée), puis **valide son travail** : la validation du document Cycle de vie des données s'appuie sur le sensor [`data-lifecycle`](../sensors/sensors/data-lifecycle.md) — ne valide pas tant qu'il n'est pas `✅` (présent + sections renseignées selon les données du projet) ; sinon, renvoie une demande de correction à Diego via le coordinateur.
