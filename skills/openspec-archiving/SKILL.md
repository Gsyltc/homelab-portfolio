...
mkdir -p openspec/archive/${TIMESTAMP}-{change-id}
```

**Example**:
```bash
# For change "add-user-auth" archived on Oct 26, 2025
mkdir -p openspec/archive/2025-10-26-add-user-auth
```
...
**Source** (`openspec/changes/add-user-auth/specs/authentication/spec-delta.md`):
```markdown
## ADDED Requirements
...
**Target** (`openspec/specs/authentication/spec.md`):
```bash
# Append to living spec
cat >> openspec/specs/authentication/spec.md << 'EOF'
...
START_LINE=$(grep -n "### Requirement: User Login" openspec/specs/authentication/spec.md | cut -d: -f1)

# Find the end (next requirement or end of file)
END_LINE=$(tail -n +$((START_LINE + 1)) openspec/specs/authentication/spec.md | \
           grep -n "^### Requirement:" | head -1 | cut -d: -f1)

# Delete old requirement
sed -i "${START_LINE},${END_LINE}d" openspec/specs/authentication/spec.md
...
# Manually edit openspec/specs/authentication/spec.md

# Add deprecation comment
echo "<!-- Requirement 'Legacy Password Reset' removed $(date +%Y-%m-%d) -->" >> openspec/specs/authentication/spec.md
...
mv openspec/changes/{change-id} openspec/archive/${TIMESTAMP}-{change-id}
```

**Verify move succeeded**:
```bash
# Check archive exists
ls -la openspec/archive/${TIMESTAMP}-{change-id}

# Check changes directory is clean
ls openspec/changes/ | grep "{change-id}"  # Should return nothing
```
...
# Review changes
git diff openspec/specs/authentication/spec.md
...
# Then archive
mv openspec/changes/add-user-auth openspec/archive/2025-10-26-add-user-auth
```
...
mv openspec/changes/add-user-auth/proposal.md openspec/archive/
...
```markdown
❌ Don't: Edit files in openspec/archive/
...
git add openspec/archive/ openspec/changes/
...
**Solution**:
```markdown
1. Search by partial name: grep -i "login" openspec/specs/**/*.md
...
2. Re-run validation: grep -n "###" openspec/specs/**/*.md
