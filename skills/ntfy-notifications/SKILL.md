...
- **Topic** : le nom du topic est stocké dans la variable d'environnement `NTFY_TOPIC` (topic de la notification)
...
```bash
curl -u "$NTFY_USER:$NTFY_PASSWORD" -d "Votre message" $NTFY_URL/$NTFY_TOPIC
```
