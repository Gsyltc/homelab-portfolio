...
test -d openspec/changes/{change-id} && echo "✓ Directory exists" || echo "✗ Directory missing"
```
...
ls openspec/changes/ | grep -i "{search-term}"
```
...
grep -c "^[0-9]\+\." openspec/changes/{change-id}/tasks.md

# Show task list
grep "^[0-9]\+\." openspec/changes/{change-id}/tasks.md
```
...
grep -n "## ADDED\|MODIFIED\|REMOVED" openspec/changes/{change-id}/specs/**/*.md
```

**Example output**:

```
openspec/changes/add-auth/specs/authentication/spec-delta.md:3:## ADDED Requirements
openspec/changes/add-auth/specs/authentication/spec-delta.md:45:## MODIFIED Requirements
```
...
grep -c "SHALL" openspec/changes/{change-id}/specs/**/*.md
```
...
CHANGE_ID="$1"
BASE_PATH="openspec/changes/$CHANGE_ID"
...
grep -n "^###\? Scenario:\|^#####+ Scenario:" openspec/changes/{change-id}/specs/**/*.md
```
...
test -f openspec/changes/$CHANGE_ID/proposal.md && \
test -f openspec/changes/$CHANGE_ID/tasks.md && \
grep -q "## ADDED\|MODIFIED\|REMOVED" openspec/changes/$CHANGE_ID/specs/**/*.md && \
grep -q "### Requirement:" openspec/changes/$CHANGE_ID/specs/**/*.md && \
grep -q "#### Scenario:" openspec/changes/$CHANGE_ID/specs/**/*.md && \
...
echo "Files: $(find openspec/changes/$CHANGE_ID -type f | wc -l)"
echo "Tasks: $(grep -c "^[0-9]\+\." openspec/changes/$CHANGE_ID/tasks.md)"
echo "Requirements: $(grep -h "### Requirement:" openspec/changes/$CHANGE_ID/specs/**/*.md | wc -l)"
echo "Scenarios: $(grep -h "#### Scenario:" openspec/changes/$CHANGE_ID/specs/**/*.md | wc -l)"
```
