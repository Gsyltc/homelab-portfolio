#!/usr/bin/env python3
"""
Générateur de modèles de pipeline ETL — ETL Pipeline Builder
Génère des modèles de pipeline ETL/ELT au format dbt / Airflow / SQL personnalisé.
Prend en charge le chargement incrémental, le CDC, la gestion des erreurs et la logique de nouvelle tentative.
"""
...
def parse_args():
    parser = argparse.ArgumentParser(description="Générateur de modèles de pipeline ETL")
...
def generate_dbt_template(args) -> str:
    """Génère un modèle de modèle dbt."""
    mode = args.mode
  ...
  config(
    ...
    if mode == "incremental":
        incremental_filter = """
{% if is_incremental() %}
  -- Logique incrémentale : ne traiter que les données ajoutées/modifiées depuis la dernière exécution
...
transformed AS (
    SELECT
        ...
        -- Champs métier (mappage personnalisé)
        id,
        name,
        description,
        amount,
        status,
        -- Clé de dimension de date
        CAST(FORMAT_DATE('%Y%m%d', created_at) AS INT64) AS fk_date_sk,
        -- Colonnes de suivi SCD Type 2
    ...
    FROM source
    WHERE 1=1
      -- Filtrage de qualité des données
      AND id IS NOT NULL
...
"""
    ...
    # Ajouter le fichier de tests dbt
...
def generate_airflow_template(args) -> str:
    """Génère un modèle de DAG Airflow."""
    ...
    if mode in ("incremental", "cdc"):
        incremental_logic = """
    # Récupérer l'horodatage de la dernière exécution réussie
...
def extract_data(**context):
    """Extrait les données depuis {args.source_type}."""
...
def transform_data(**context):
    ...
    df = df.dropna(subset=["id", "name"])  # Supprimer les lignes dont les colonnes clés sont vides
...
def load_data(**context):
    """Charge les données dans {args.target_platform}."""
    import pandas as pd
...
def generate_custom_sql_template(args) -> str:
    """Génère un modèle de pipeline SQL personnalisé."""
    mode = args.mode
...
"""
    ...
    if args.target_platform == "bigquery":
        # Step 1: Extraction vers la zone de staging
        sql += f"""
...
def main():
    ...
    print(f"✅ Modèle de pipeline ETL généré : {output_file}")
    print(f"📋 Nom du pipeline : {args.pipeline_name}")
    print(f"📥 Source : {args.source_type}.{args.source_table}")
    print(f"📤 Cible : {args.target_platform}.{args.target_table}")
    print(f"🔄 Mode : {args.mode}")
    print(f"🔧 Orchestrateur : {args.orchestrator}")
