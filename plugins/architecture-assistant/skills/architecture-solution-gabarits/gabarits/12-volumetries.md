...
>
> Chaque volume est identifié (`VOL-001`, …), relié au **composant** concerné du `06-architecture-solutions.md` (ex. `SYS-001`) et aux **types de données** du `10-cycle_vie_donnees.md` (ex. `DON-001`). Les volumétries **alimentent le dimensionnement** (`09-deploiement.md`) et les **tests de charge** (`13-plan-qualite.md`). Les seuils de **monitoring/alertes** sont des seuils d'alerte concrets (même provisoires) à confirmer en conception détaillée.
