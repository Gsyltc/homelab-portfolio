...
> Les NFRs doivent être **mesurables et testables** (sans métrique, elles ne sont pas vérifiables). Elles sont validées par les tests non-fonctionnels du plan de qualité (`13-plan-qualite.md`).
...
> **Le Well-Architected Framework se base sur les piliers Azure et AWS** : [Azure Well-Architected Framework](https://learn.microsoft.com/en-us/azure/well-architected/) et [AWS Well-Architected Framework](https://aws.amazon.com/architecture/well-architected/). Chaque pilier est évalué selon les deux référentiels.
> Chaque section n'est remplie que si le pilier est pertinent pour le projet. Chaque patron retenu fait l'objet d'une fiche dans le répertoire `patron-architecture/` de la skill `architecture-solution-gabarits`, est décrit dans `06-architecture-solutions.md` et tracé dans un ADR.

#### Excellence opérationnelle

- **Patron** : besoin couvert, lien avec le pilier.
