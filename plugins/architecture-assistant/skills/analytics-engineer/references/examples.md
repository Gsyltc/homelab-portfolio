# Analytics Engineer — Exemples de code
...
```python
# Script Python pour le rafraîchissement automatisé de la BI
import requests
import json
...
class BIRefreshManager:
    ...
    def authenticate(self):
        """Authentification auprès de Tableau Server"""
    ...
    def refresh_datasource(self, datasource_id):
        """Rafraîchir une source de données spécifique"""
        headers = {
    ...
    def check_job_status(self, job_id):
        """Vérifier le statut d'une tâche de rafraîchissement"""
...
# Automatisation de l'exécution dbt avec rafraîchissement de la BI
def run_dbt_and_refresh_bi():
    """Exécuter les modèles dbt et rafraîchir les tableaux de bord BI"""
    import subprocess
    ...
    try:
        ...
        if dbt_result.returncode == 0:
            ...
            if test_result.returncode == 0:
                ...
                # Rafraîchir les tableaux de bord BI
...
class DataMonitor:
    ...
    def check_data_freshness(self, table_name, timestamp_column, max_age_hours=2):
        """Vérifier si les données sont suffisamment fraîches"""
        query = f"""
        SELECT 
    ...
    def check_row_count_anomaly(self, table_name, threshold_percent=20):
        """Vérifier les variations inhabituelles du nombre de lignes"""
        query = f"""
...
# Exécuter les vérifications de surveillance
tables_to_monitor = [
    'dim_customers',
    'fct_orders',
    'fct_web_events'
