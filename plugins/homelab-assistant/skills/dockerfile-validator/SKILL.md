...
> **Périmètre Homelab (A7).** Dans le workflow Homelab, cette skill s'applique **uniquement aux stacks *build-from-source*** — celles qui construisent une image localement (`build:` dans le `docker-compose`, présence d'un `Dockerfile`). Pour une stack qui tire une **image déjà publiée** (registry public/privé, pas de `Dockerfile` à construire), `dockerfile-validator` **ne s'applique pas** ; le durcissement passe alors par la skill `docker-composer` et la revue de l'Analyste QA.
