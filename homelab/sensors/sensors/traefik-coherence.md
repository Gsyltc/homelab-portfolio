...
Check déterministe déclenché **au gate de phase** (`fire_on: gate`) : vérifie la **cohérence Traefik** du docker-compose livré (labels de routage, entrypoints, appartenance au réseau Traefik). Ce sensor **ne réimplémente pas** l'analyse Traefik : il **référence le check `traefik-manager-read` existant** (skill de l'Analyste QA, §2.2) et se limite à consigner un signal factuel advisory. **Advisory** (`default_severity: advisory`).
...
checks:
  reference_check_existant:
    check: traefik-manager-read              # autorité : skill de l'Analyste QA (§2.2)
...
Advisory : consigne le signal Traefik, ne bloque pas. **L'autorité technique reste `traefik-manager-read`** exécuté par l'Analyste QA (§2.2) ; ce sensor ne fait que tracer le verdict et un contrôle de surface, il **ne remplace pas** la vérification de l'Analyste QA ni la validation humaine (SG-3). **Parsing statique / lecture seule** (SG-4) : jamais d'écriture sur la configuration Traefik.
