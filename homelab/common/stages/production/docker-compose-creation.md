...
Produire le fichier (skill `docker-composer`), conserver les commentaires `#` des gabarits, vérifier la syntaxe YAML, déposer le livrable **téléchargeable** (`multica attachment upload`) et **mentionner le Tech Lead** (mention valide) avec le compte-rendu JSON en pièce jointe, au **format unique** [`report-format.md`](../../protocols/report-format.md) (corps de commentaire minimal).
