...
L'agent de revue notifie en retour le Tech Lead par **mention valide** sur l'issue, **en attachant le compte-rendu JSON** de revue (`review-<agent>-<stack>.json`) conforme au format unique [`report-format.md`](report-format.md) / [`report-format.schema.json`](report-format.schema.json). Ce geste est **identique à chaque verdict — `OK` compris** : un verdict `OK` (re-contrôle final réussi) exige lui aussi la mention de retour **et** le `report.json` en pièce jointe, exactement comme `RENVOI` / `BLOQUE`. Le corps du commentaire est réduit au strict minimum (mention valide + `verdict` en un mot + « rapport JSON en pièce jointe ») : aucune prose ni liste de points dans le thread, le détail vit dans le JSON attaché. Une revue n'est jamais close sans cette notification à mention valide (sinon compte-rendu réputé non rendu, flux arrêté). **L'agent de revue vérifie `trigger_outcomes` après publication** : tant qu'un run de reprise du Tech Lead n'est pas confirmé enfilé, la revue reste **ouverte** — reprise unique de la mention, puis, en cas d'échec, passage de l'issue en `blocked` et escalade humaine. **L'omission de la mention de retour est un défaut de relais à signaler, jamais une clôture implicite.**
