...
- Le compte-rendu suit le **format unique** [`homelab/common/protocols/report-format.md`](../common/protocols/report-format.md) : dépose le JSON conforme (`review-qa-docker-<stack>.json`) en **pièce jointe** (`--attachment`) — problèmes triés par gravité (`critical` en tête), avec `constat` / `cause` / `correction` par point ; le corps du commentaire est réduit au strict minimum (mention valide + `verdict` + « rapport JSON en pièce jointe »).
