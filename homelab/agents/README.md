...
| Analyste QA | [analyste-qa-agent.md](analyste-qa-agent.md) | Vérifie les livrables produits par les spécialistes — docker-compose (YAML, Swarm, hardening technique, Traefik) et configuration Terraform `.tfvars` (structure HCL, variables, absence de `${SNI}`/secret) — et renvoie les défauts à leur créateur (Spécialiste Docker ou Spécialiste Terraform ; contrôle, sans correction). Intervient après création. |
