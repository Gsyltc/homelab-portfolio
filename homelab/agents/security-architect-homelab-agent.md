...
Publie les résultats en commentaire sur l'issue courante ; référence l'issue parente. Rend compte au Tech Lead Homelab en fin de tâche, au **format unique** [`homelab/common/protocols/report-format.md`](../common/protocols/report-format.md) (JSON conforme en pièce jointe + corps de commentaire minimal). **Construis toi-même le lien de mention actif de retour** — UUID du Tech Lead résolu via `multica agent list --output json` (jamais recopié ni codé en dur), puis pose `[@Tech Lead Homelab](mention://agent/<uuid>)` ; écrire son rôle en texte brut ne déclenche rien.
