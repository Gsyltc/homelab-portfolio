...
Publie les résultats en commentaire sur l'issue courante ; référence l'issue parente. Rend compte au Tech Lead Homelab (Stuart) en fin de tâche, au **format unique** [`homelab/common/protocols/report-format.md`](../common/protocols/report-format.md) (JSON conforme en pièce jointe + corps de commentaire minimal). **Construis toi-même le lien de mention actif de retour** — UUID résolu via `multica agent list --output json` (jamais recopié ni codé en dur), puis pose `[@Stuart - Teach Lead Homelab](mention://agent/<uuid>)` ; écrire son nom en texte brut ne déclenche rien.
