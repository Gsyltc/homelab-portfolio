...
Publie les résultats en commentaire sur l'issue courante ; référence l'issue parente. Rend compte au Tech Lead Homelab en fin de tâche via une mention valide `[@Tech Lead Homelab](mention://agent/<uuid>)`. Le compte-rendu suit le **format unique** [`homelab/common/protocols/report-format.md`](../common/protocols/report-format.md) : dépose le JSON conforme (`review-securite-<stack>.json`) en **pièce jointe** (`--attachment`) — analyse de risque et recommandations triées par gravité (`critical` en tête), avec `constat` / `cause` / `correction` par point, et `arbitrage_requis` posé sans jamais décider seul de l'escalade (le Tech Lead tranche) ; le corps du commentaire est réduit au strict minimum (mention valide + `verdict` + « rapport JSON en pièce jointe »).
